...
static uint8_t frame_bitmap[131072] = {0};
...
void frame_mark_free(int frame)
{
    ...
    int index = frame / 8;
    int shift = frame % 8;
    frame_bitmap[index] = frame_bitmap[index] & ~(1UL << shift);
    frame_count++;
}
...
void *frame_alloc()
{
    for (int i = 0; i < 131072 * 8; i++)
    {
        if (is_frame_free(i))
        {
            term_printf("found frame %d free\n", i);
            frame_mark_used(i);
...
void frame_init(uint_t RAM_in_KB)
{
    ...
    uint32_t last_ram_frame = ADDR_TO_FRAME_NB(RAM_in_BYTES);

    frame_count = 0;
    for (uint32_t i = last_module_frame; i <= last_ram_frame; i++)
    {
        frame_mark_free(i);
    }

    term_printf("%d %d\n", last_module_frame, last_ram_frame);
