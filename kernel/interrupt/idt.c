...
{
	// TODO
	// Note that the regs_t structure in argument gives you access
	// to interesting fields such as the exception number.
	term_setfgcolor(WHITE);
	term_setbgcolor(BLACK);
	term_printf("%s\n", exception_names[regs->number]);
	halt();
}
...
void irq_handler(regs_t *regs)
{
	// TODO
	// Note that the regs_t structure in argument gives you access
	// to interesting fields such as the IRQ number.
	handler_t *h = irq_get_handler(regs->number);
	h->func();
	pic_eoi(regs->number);
}
